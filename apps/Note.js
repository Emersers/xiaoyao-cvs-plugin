...
//#体力
export async function Note(e, { render }, poke) {
  ...
  if (isV3) {
    if (e.msg.replace(/全|全部/g, "多").includes("多")) {
      ...
      for (let item of Object.keys(ck)) {
        let res = await (
          await e.runtime.createMysApi(ck[item].uid, ck[item].ck)
        ).getData("dailyNote");
...
async function getDailyNote(uid, cookie) {
  ...
  let { url, headers, query, body } = mysApi.getUrl("dailyNote", uid);
...
export async function DailyNoteTask() {
  ...
  for (let item in notes.noteCfg) {
    ...
    for (let i of taskUser) {
      ...
      let render;
      if (isV3) {
        let { getRender } = await import(
          `file://${_path}/plugins/xiaoyao-cvs-plugin/adapter/render.js`
        );
...
export async function Note_appoint(e) {
  ...
  if (msg.includes("列表")) {
    ...
    let count = 0;
    let msgData = [
      `模板列表共，第${xlmsg}页，共${listLength}张，\n您可通过【#体力模板设置1】来绑定你需要的体力模板~\n请选择序号~~\n当前支持选择的模板有:`
    ];
